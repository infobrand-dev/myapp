...
- Central tenant registry now starts moving toward `1 tenant = 1 PostgreSQL schema`: `tenants` may map to `tenant_servers`, `tenant_databases`, and `tenant_domains`, while request bootstrapping may switch the default runtime DB connection to a tenant-scoped connection and set PostgreSQL `search_path` to `<tenant_schema>, public`.
- Runtime default saat ini harus tetap `column`: koneksi utama tetap dari `.env`, tabel operasional tenant tetap shared, dan isolasi aktif tetap lewat `tenant_id`. Registry topology (`server/database/schema`) disimpan lebih dulu agar switch ke mode `schema` atau `database` nanti tidak perlu membongkar ulang model registry.
- Source of truth topology tenant sekarang harus dibaca dari `tenant_topologies`, yang mengikat setiap tenant ke `tenant_servers` + `tenant_databases` serta menyimpan `server_key`, `database_key`, `schema_name`, `isolation_mode`, dan `status`.
- Untuk kesiapan scale-out penuh, topology tenant juga harus memiliki lapisan runtime (`tenant_runtime_topologies` ke `app_servers`) dan storage (`tenant_storage_topologies` ke `storage_servers` + `storage_buckets`). Runtime default tetap shared, tetapi registry-nya sudah harus ada agar tenant enterprise nanti bisa diarahkan ke cluster app atau storage yang berbeda tanpa redesign ulang.
- Control-plane storage wajib tetap central: `storage_profiles`, `storage_servers`, `storage_buckets`, `tenant_topologies`, `tenant_runtime_topologies`, dan `tenant_storage_topologies` harus dibaca dari koneksi `central`. Data operasional file seperti `stored_files` dan `stored_file_access_logs` tetap tenant-scoped/shared untuk mode `tenant_id` saat ini.
- Upload tenant tidak boleh lagi memilih storage hanya dari disk global. `App\Services\StorageRoutingService` harus resolve `tenant_storage_topologies` aktif per tenant + visibility, lalu menyimpan snapshot lokasi aktual ke `stored_files.storage_snapshot` agar historical read tetap valid saat topology berubah.
- Queue tenant sekarang harus membawa snapshot topology, minimal `tenant_id`, `isolation_mode`, `server_key`, `database_key`, `schema_name`, `app_server_key`, `queue_cluster`, `storage_topology_fingerprint`, dan `topology_fingerprint`. Saat worker memproses job tenant, snapshot ini harus divalidasi fail-closed terhadap registry aktif.
- Central/platform models that must stay global should use the explicit `central` connection, while operational modules remain compatible dengan shared runtime sampai mode tenancy dinaikkan secara sengaja.
- Ownership contract harus dibekukan di satu tempat (`App\Multitenancy\TenantOwnershipManifest`): central/platform tables tetap global, sedangkan data operasional tenant tetap dianggap tenant-scoped walau runtime aktif masih `tenant_id`.
- Query guard tenant sekarang harus fail-closed untuk entrypoint tenant-scoped penting. Shared helper `App\Multitenancy\QueryContextGuard` dipakai untuk memaksa `TenantContext` dan, bila relevan, `CompanyContext` sebelum query tenant berjalan.
...
- Tenant subdomain root is now the primary `brand page` surface for commerce creator. Brand identity, CTA links, FAQ, testimonials, and fixed section ordering live under `tenant.meta.commerce_creator.brand`, while apex marketing remains outside tenant route space.
- Public product selling fields live in `product.meta.public_offer`. Reuse `products` as the selling source of truth; do not introduce a parallel `offers` table unless the product domain genuinely stops fitting.
...
- Commerce creator post-payment delivery should continue to reuse `sales.meta.commerce`. Buyer-access state, affiliate attribution snapshot, platform fee snapshot, and wallet settlement snapshot belong there so checkout, order page, email, affiliate conversion, and wallet posting all read one envelope.
- Tenant-side affiliate tracking is a separate module/domain from platform SaaS affiliate. Use `affiliate_*` tables for tenant commerce referrals and keep `platform_*` affiliate tables reserved for Meetra SaaS plan sales.
- Commerce affiliate now assumes `account-based Meetra affiliators`, not anonymous external partners. Seller tenant may open selected products for affiliate, another authenticated Meetra user may claim that source product into their workspace as an affiliate listing without duplicating the underlying `products` row, and only listing/landing metadata may be customized on the affiliate side.
- Tenant wallet balance must not be stored in `finance_accounts`. Use immutable `wallet_ledger_entries` as the source of truth, with `wallet_accounts` and `wallet_payout_requests` as supporting state for settlement and manual payout review.
