...
use App\Modules\Finance\Actions\ReopenAccountingPeriodClosingAction;
use App\Modules\Finance\Http\Requests\ReopenAccountingPeriodClosingRequest;
...
class AccountingPeriodClosingController extends Controller
{
    public function index(): View
    {
        return view('finance::governance.period-closings', [
            'closings' => AccountingPeriodClosing::query()
                ...
                ->with(['closingJournal.reversals', 'reopeningJournal', 'periodLock', 'closer', 'reopener'])
                ->latest('period_end')
                ->paginate(20),
    ...
    public function reopen(
        AccountingPeriodClosing $accountingPeriodClosing,
        ReopenAccountingPeriodClosingRequest $request,
        ReopenAccountingPeriodClosingAction $action
    ): RedirectResponse {
        $closing = $action->execute($accountingPeriodClosing, $request->validated(), $request->user());

        return redirect()
            ->route('finance.period-closings.index')
            ->with('status', 'Period closing berhasil di-reopen. Reversal journal: ' . ($closing->reopeningJournal ? $closing->reopeningJournal->journal_number : '-'));
    }
}
