...
@php
    $selectedBranchId = old('branch_id', $transaction->branch_id);
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <div class="form-hint">Pastikan type transaksi sesuai tujuan pencatatan. `Expense` khusus untuk biaya operasional.</div>
                    @error('transaction_type') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">Date <span class="text-danger">*</span></label>
                    <input type="datetime-local" name="transaction_date" class="form-control @error('transaction_date') is-invalid @enderror"
                    ...
                    <select name="finance_account_id" id="finance-account-select" class="form-select @error('finance_account_id') is-invalid @enderror" required size="6">
                        ...
                        @foreach($accounts as $account)
                            <option value="{{ $account->id }}"
                                    ...
                                    @selected((string) old('finance_account_id', $transaction->finance_account_id) === (string) $account->id)>{{ $account->name }} ({{ \App\Modules\Finance\Models\FinanceAccount::typeOptions()[$account->account_type] ?? $account->account_type }})</option>
                        @endforeach
                    </select>
                    <div class="form-hint">Account menentukan bucket dana transaksi ini: kas, bank, atau e-wallet.</div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-hint">Category difilter otomatis berdasarkan type agar tidak terjadi mismatch.</div>
                ...
                <div class="col-md-3">
                    ...
                    <div class="form-hint">
                        @if($usesAllBranchView)
                            Anda sedang melihat mode `Semua branch`. Jika dikosongkan, transaksi akan tetap diarahkan ke branch operasional default{{ $defaultBranchLabel ? ': '.$defaultBranchLabel : '' }}.
                        @else
                            Anda bisa pindahkan transaksi ini ke branch lain dalam company yang sama bila memang perlu.
                        @endif
                    </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="4">{{ old('notes', $transaction->notes) }}</textarea>
                    @error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
            </div>
        </div>
        <div class="card-footer d-flex justify-content-end gap-2">
            <a href="{{ route('finance.transactions.show', $transaction) }}" class="btn btn-outline-secondary">Batal</a>
