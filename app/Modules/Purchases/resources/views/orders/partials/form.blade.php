@php
    ...
    $selectedPurchaseTaxId = old('tax_rate_id', data_get($order->meta, 'tax.tax_rate_id'));
    ...
    $cancelRoute = $order->exists ? route('purchases.orders.show', $order) : route('purchases.orders.index');
    $purchasablesByKey = collect($purchasables)->keyBy('key');
@endphp

<script>
window._purchasables = @json(collect($purchasables)->map(fn($p) => [
    'key' => $p['key'],
    'label' => $p['label'],
    ...
    <div class="row g-3">
        <div class="col-lg-4">
            <div class="card h-100">
                <div class="card-header"><h3 class="card-title">Purchase Order Info</h3></div>
                <div class="card-body row g-3">
                    <div class="col-12">
                        <x-contact-select
                            ...
                            :value-name="$order->supplier?->name"
                            :value-type="$order->supplier?->type"
                        />
                    </div>
                    ...
                    <div class="col-md-6"><label class="form-label">Expected Receive Date</label><input type="date" name="expected_receive_date" class="form-control @error('expected_receive_date') is-invalid @enderror" value="{{ old('expected_receive_date', optional($order->expected_receive_date)->format('Y-m-d')) }}">@error('expected_receive_date') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                    ...
                    <div class="col-md-6"><label class="form-label">Landed Cost</label><input type="number" min="0" step="0.01" name="landed_cost_total" class="form-control @error('landed_cost_total') is-invalid @enderror" value="{{ old('landed_cost_total', data_get($orderTotals, 'landed_cost_total', $order->landed_cost_total ?? 0)) }}">@error('landed_cost_total') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                    ...
                    @endif
                    <div class="col-12"><label class="form-label">Notes</label><textarea name="notes" rows="2" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $order->notes) }}</textarea>@error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                    <div class="col-12"><label class="form-label">Internal Notes</label><textarea name="internal_notes" rows="2" class="form-control @error('internal_notes') is-invalid @enderror">{{ old('internal_notes', $order->internal_notes) }}</textarea>@error('internal_notes') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header"><h3 class="card-title">Order Items</h3><div class="card-options"><button type="button" class="btn btn-sm btn-outline-primary" data-add-purchase-item>Add Item</button></div></div>
                <div class="card-body">
                    @if($errors->any())
                ...
                <div class="card-footer d-flex justify-content-end gap-2"><a href="{{ $cancelRoute }}" class="btn btn-outline-secondary">Cancel</a><button type="submit" class="btn btn-primary">Save Purchase Order</button></div>
            </div>
        </div>
    </div>
</form>
