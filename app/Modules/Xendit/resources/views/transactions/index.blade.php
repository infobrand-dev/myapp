...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h2 class="mb-0">Xendit Transactions</h2>
        <div class="text-muted small">Riwayat invoice checkout via Xendit.</div>
    </div>
    <a href="{{ route('xendit.settings.edit') }}" class="btn btn-outline-secondary">Settings</a>
</div>

@if(!$isConfigured)
    <div class="alert alert-warning">
        Xendit belum dikonfigurasi atau tidak aktif. <a href="{{ route('xendit.settings.edit') }}">Atur sekarang</a>
    </div>
@endif
    ...
    <div class="card-body">
        <form method="GET" class="row g-2 align-items-end">
            <div class="col-md-5">
                ...
                <input type="text" name="search" class="form-control" value="{{ $filters['search'] ?? '' }}" placeholder="Reference, invoice ID, customer...">
            </div>
            <div class="col-md-3">
                <label class="form-label">Status</label>
                <select name="status" class="form-select">
                    <option value="">Semua</option>
                    @foreach(['PENDING', 'PAID', 'SETTLED', 'EXPIRED', 'FAILED'] as $status)
            ...
            <tbody>
                @forelse($transactions as $tx)
                    <tr>
                        ...
                        <td>
                            <div class="font-monospace small">{{ $tx->external_reference }}</div>
                            @if($tx->invoice_id)
                                <div class="text-muted small">{{ $tx->invoice_id }}</div>
                            @endif
                        </td>
                        <td>
                        ...
                        <td><a href="{{ route('xendit.transactions.show', $tx) }}" class="btn btn-sm btn-outline-secondary btn-icon"><i class="ti ti-eye icon"></i></a></td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="text-center text-muted">Belum ada transaksi Xendit.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
