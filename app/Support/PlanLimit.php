<?php

namespace App\Support;

class PlanLimit
{
    public const COMPANIES = 'max_companies';
    public const BRANCHES = 'max_branches';
    public const USERS = 'max_users';
    public const PRODUCTS = 'max_products';
    public const CONTACTS = 'max_contacts';
    public const WHATSAPP_INSTANCES = 'max_whatsapp_instances';
    public const SOCIAL_ACCOUNTS = 'max_social_accounts';
    public const LIVE_CHAT_WIDGETS = 'max_live_chat_widgets';
    public const CHATBOT_ACCOUNTS = 'max_chatbot_accounts';
    public const EMAIL_INBOX_ACCOUNTS = 'max_email_inbox_accounts';
    public const EMAIL_CAMPAIGNS = 'max_email_campaigns';
    public const WA_BLAST_RECIPIENTS_MONTHLY = 'max_wa_blast_recipients_monthly';
    public const EMAIL_RECIPIENTS_MONTHLY = 'max_email_recipients_monthly';
    public const AI_CREDITS_MONTHLY = 'max_ai_credits_monthly';
    public const CHATBOT_KNOWLEDGE_DOCUMENTS = 'max_chatbot_knowledge_documents';
    public const AUTOMATION_WORKFLOWS = 'max_automation_workflows';
    public const AUTOMATION_EXECUTIONS_MONTHLY = 'max_automation_executions_monthly';
}
