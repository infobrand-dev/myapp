...
<section class="py-4">
    <div class="container">
        <div class="landing-trust-strip">
            <div class="landing-trust-item"><i class="ti ti-brand-whatsapp"></i><div class="small">WhatsApp API Resmi</div></div>
            <div class="landing-trust-sep"></div>
            <div class="landing-trust-item"><i class="ti ti-lock"></i><div class="small">Data tiap tenant terisolasi</div></div>
            <div class="landing-trust-sep"></div>
            <div class="landing-trust-item"><i class="ti ti-credit-card"></i><div class="small">Bayar lokal — VA, QRIS, transfer</div></div>
            <div class="landing-trust-sep"></div>
            <div class="landing-trust-item"><i class="ti ti-headset"></i><div class="small">Support berbahasa Indonesia</div></div>
            <div class="landing-trust-sep"></div>
            <div class="landing-trust-item"><i class="ti ti-building-store"></i><div class="small">Multi-branch & multi-company</div></div>
        </div>
    </div>
</section>
                ...
                <div class="pricing-tab-nav">
                    @foreach ($intervalOrder as $iKey)
                        ...
                        <button type="button" class="pricing-tab-btn {{ $loop->first ? 'active' : '' }}" data-tab="{{ $iKey }}">
                            {{ $iMeta['label'] }}
                            @if($iMeta['badge'])<span class="pricing-tab-badge">{{ $iMeta['badge'] }}</span>@endif
                        </button>
                        @endif
                    @endforeach
                </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-4">
                        <div class="landing-plan-card p-4 p-lg-5 {{ $recommended ? 'featured' : '' }}">
                            @if($recommended)<div class="landing-plan-popular">Paling populer</div>@endif
                            <div class="h3 mb-0 fw-800 mb-1">{{ $sales['display_name'] ?? $plan->display_name }}</div>
                            ...
                            @if($fit)<div class="landing-package-fit mb-4"><i class="ti ti-user-circle"></i>{{ $fit }}</div>@endif
                            ...
                            <div class="landing-limit-table mb-4">
                                @if($users !== null)<div class="landing-limit-row"><span>Pengguna (CS, Admin)</span><span class="fw-semibold">{{ number_format((int)$users, 0, ',', '.') }}</span></div>@endif
                                @if($contacts !== null)<div class="landing-limit-row"><span>Kontak</span><span class="fw-semibold">{{ number_format((int)$contacts, 0, ',', '.') }}</span></div>@endif
                                @if($socialAcc !== null)<div class="landing-limit-row"><span>Akun sosial media</span><span class="fw-semibold">{{ number_format((int)$socialAcc, 0, ',', '.') }}</span></div>@endif
                                @if($widgets !== null)<div class="landing-limit-row"><span>Widget live chat</span><span class="fw-semibold">{{ number_format((int)$widgets, 0, ',', '.') }}</span></div>@endif
                                @if(($hasWaApi || $hasWaWeb) && $waInst !== null)<div class="landing-limit-row"><span>Koneksi WhatsApp</span><span class="fw-semibold">{{ number_format((int)$waInst, 0, ',', '.') }}</span></div>@endif
                                @if($hasAi && $aiCredits > 0)<div class="landing-limit-row"><span>AI Credits / bulan <a href="#ai-credits" class="landing-credit-hint">Apa ini?</a></span><span class="fw-semibold">{{ number_format($aiCredits, 0, ',', '.') }}</span></div>@endif
                            </div>
                            @if(!empty($sales['highlights']))
                            <div class="landing-checklist small text-muted">
                                @foreach ($sales['highlights'] as $hl)<div><i class="ti ti-check text-success"></i> {{ $hl }}</div>@endforeach
                            </div>
                            @endif
                            @if(!$hasAi)<div class="landing-plan-note mt-3"><i class="ti ti-info-circle"></i> Chatbot AI tersedia mulai paket Growth ke atas.</div>@endif
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="landing-panel rounded-4 p-4 p-lg-5">
            <div class="row g-5 align-items-start">
                <div class="col-lg-5">
                    ...
                    <p class="landing-subtext mb-4">AI Credits adalah satuan penggunaan fitur kecerdasan buatan di workspace Anda. Setiap kali bot AI menjawab pesan atau mencari referensi dari dokumen, kredit berkurang sesuai pemakaian.</p>
                    <p class="text-muted small">Credits direset setiap bulan sesuai paket. Jika habis, Anda bisa top up kapan saja dari halaman subscription tanpa perlu upgrade paket.</p>
                </div>
                <div class="col-lg-7">
                    <div class="row g-3">
                        <div class="col-sm-6">
                    ...
                    <div class="landing-credit-note mt-3">
                        ...
                        <div><strong>Tips:</strong> Chatbot dengan mode "Berbasis Aturan" (rule-only) <em>tidak menggunakan</em> AI Credits sama sekali. Credits hanya terpakai saat bot benar-benar memanggil model AI untuk menjawab.</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-7">
                <div class="landing-steps-grid">
                    <div class="landing-step">
                        ...
                        <div><h3 class="h6 mb-1">Pilih paket</h3><p class="text-muted small mb-0">Pilih Starter, Growth, atau Scale sesuai channel dan kapasitas tim Anda.</p></div>
                    </div>
                    <div class="landing-step">
                        <div class="landing-step-number">2</div>
                        <div><h3 class="h6 mb-1">Buat workspace</h3><p class="text-muted small mb-0">Isi nama bisnis, subdomain, dan informasi admin utama. Selesai dalam 2 menit.</p></div>
                    </div>
                    <div class="landing-step">
                        <div class="landing-step-number">3</div>
                        <div><h3 class="h6 mb-1">Bayar invoice</h3><p class="text-muted small mb-0">Pilih metode pembayaran lokal — transfer bank, virtual account, atau QRIS.</p></div>
                    </div>
                    <div class="landing-step">
                        <div class="landing-step-number">4</div>
                        <div><h3 class="h6 mb-1">Langsung bisa dipakai</h3><p class="text-muted small mb-0">Setelah pembayaran dikonfirmasi, workspace aktif dan semua modul sesuai paket siap digunakan.</p></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="landing-panel p-4 p-lg-5 rounded-4">
            <div class="row g-4 align-items-center">
                <div class="col-lg-8">
                    ...
                    <p class="landing-subtext mb-0">Gunakan URL seperti <strong>namaworkspace.{{ config('multitenancy.saas_domain') }}/login</strong>, atau klik tombol di samping untuk mencari workspace berdasarkan email atau subdomain.</p>
                </div>
