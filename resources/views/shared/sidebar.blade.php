<aside class="navbar navbar-vertical navbar-expand-lg border-end" style="min-height: 100vh;">
    <div class="container-fluid">
        ...
        <ul class="navbar-nav pt-lg-3">
            ...
            @endif
            @if(config('modules.whatsapp_api.enabled') ?? true)
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle d-flex align-items-center justify-content-start gap-2 px-3 py-2 rounded-2 text-start w-100 {{ request()->routeIs('whatsapp-api.*') ? 'active bg-primary-lt text-primary' : 'bg-body' }}" href="#" data-bs-toggle="dropdown" aria-expanded="false">
                    <span class="nav-link-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                            <path d="M7 8h10m-5 -5v10" />
                            <path d="M6.5 14h11l-3 6h-5z" />
                        </svg>
                    </span>
                    <span class="nav-link-title">WhatsApp API</span>
                </a>
                <div class="dropdown-menu show position-static border-0 shadow-none px-0 py-1 ms-4">
                    <a class="dropdown-item px-3 {{ request()->routeIs('whatsapp-api.chat') ? 'active' : '' }}" href="{{ route('whatsapp-api.chat') }}">Chat</a>
                    <a class="dropdown-item px-3 {{ request()->routeIs('whatsapp-api.settings') ? 'active' : '' }}" href="{{ route('whatsapp-api.settings') }}">Settings</a>
                </div>
            </li>
            @endif
            @if(config('modules.whatsapp_api.enabled'))
            <li class="nav-item">
                <a class="nav-link d-flex align-items-center justify-content-start gap-2 px-3 py-2 rounded-2 text-start w-100 {{ request()->routeIs('whatsapp-api.*') ? 'active bg-primary-lt text-primary' : 'bg-body' }}" href="{{ route('whatsapp-api.settings') }}">
                    <span class="nav-link-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                            <path d="M3 21l1.65 -3.8a9 9 0 1 1 3.4 3.4l-4.05 .4" />
